from __future__ import annotations

from src.config.settings import get_app_settings
from src.domain.models import IntegrationCheckResult, IntegrationCheckStatus, IntegrationPlatform
from src.infrastructure.youtube.yt_client import YTClient
from src.infrastructure.youtube.yt_fake_client import YTClientFake


def _build_yt_client() -> YTClient:
    settings = get_app_settings()
    return YTClient() if settings.is_production_env else YTClientFake()


class YouTubeIntegrationChecker:
    @property
    def platform_name(self) -> IntegrationPlatform:
        return IntegrationPlatform.YOUTUBE

    @property
    def is_configured(self) -> bool:
        settings = get_app_settings()
        return bool(settings.yt_client_secret_file and settings.yt_auth_user_id)

    @property
    def is_publish_target(self) -> bool:
        return True

    async def check_connection(self) -> IntegrationCheckResult:
        if not self.is_configured:
            return IntegrationCheckResult(
                platform=self.platform_name,
                status=IntegrationCheckStatus.NOT_CONFIGURED,
                is_configured=False,
                is_publish_target=self.is_publish_target,
                message="Missing YouTube OAuth configuration.",
            )

        try:
            client = _build_yt_client()
            uploads_playlist_id = await client.check_connection()
            if uploads_playlist_id:
                return IntegrationCheckResult(
                    platform=self.platform_name,
                    status=IntegrationCheckStatus.OK,
                    is_configured=True,
                    is_publish_target=self.is_publish_target,
                    message="YouTube channel access verified.",
                )
            return IntegrationCheckResult(
                platform=self.platform_name,
                status=IntegrationCheckStatus.ERROR,
                is_configured=True,
                is_publish_target=self.is_publish_target,
                message="Could not resolve YouTube uploads playlist.",
            )
        except Exception as exc:  # noqa: BLE001
            return IntegrationCheckResult(
                platform=self.platform_name,
                status=IntegrationCheckStatus.ERROR,
                is_configured=True,
                is_publish_target=self.is_publish_target,
                message=str(exc),
            )
